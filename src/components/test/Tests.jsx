...
export function Test11() {
  ...
  const [checked, setChecked] = useState(new Array(5));
  for (let i = 0; i < checked.length; i++) {
    checked[i] = context.answerIndexes[10]?.includes(i)
  }
  const objects = [{ intelligence: 1, team: 1 }, { strength: 2 }, { team: 2 }, { intelligence: 2 }, {}];
  const set = (number, e) => {
    checked[number] = e.target.checked;
    let finalObject = { intelligence: 0, strength: 0, team: 0 }
    let finalIndexArray = new Array();
    for (let i = 0; i < checked.length; i++) {
      if (checked[i]) {
        finalIndexArray.push(i);
        if (typeof (objects[i].intelligence) === 'number') finalObject.intelligence += objects[i].intelligence;
        if (typeof (objects[i].strength) === 'number') finalObject.strength += objects[i].strength;
        if (typeof (objects[i].team) === 'number') finalObject.team += objects[i].team;
      }
    }
    if (finalIndexArray.length <= 0) finalIndexArray = undefined;
    setObject(finalObject, 10, finalIndexArray, context);
  }

  /*if (typeof(context.answerIndexes[10]) !== 'object') {
    setObject({}, 10, [], context);
    console.log('created');
  }*/

  return (
    <>
      <p>Vaše skupina se chystá na výpravu a musíte si pečlivě vybrat 2 předměty, které si vezmete se sebou. Kterým vybavením se vybavíte?</p>
      <Form>
        <Form.Check readOnly disabled={!context.answerIndexes[10]?.includes(0) && context.answerIndexes[10]?.length > 1} checked={context.answerIndexes[10]?.includes(0)} value={"1"} onClick={(e) => set(0, e)} name='t11' type='checkbox' label="Mapa" required />
        <Form.Check readOnly disabled={!context.answerIndexes[10]?.includes(1) && context.answerIndexes[10]?.length > 1} checked={context.answerIndexes[10]?.includes(1)} value={"2"} onClick={(e) => set(1, e)} name='t11' type='checkbox' label="Sekera" required />
        <Form.Check readOnly disabled={!context.answerIndexes[10]?.includes(2) && context.answerIndexes[10]?.length > 1} checked={context.answerIndexes[10]?.includes(2)} value={"3"} onClick={(e) => set(2, e)} name='t11' type='checkbox' label="Vysílačky" required />
        <Form.Check readOnly disabled={!context.answerIndexes[10]?.includes(3) && context.answerIndexes[10]?.length > 1} checked={context.answerIndexes[10]?.includes(3)} value={"4"} onClick={(e) => set(3, e)} name='t11' type='checkbox' label="Provaz" required />
        <Form.Check readOnly disabled={!context.answerIndexes[10]?.includes(4) && context.answerIndexes[10]?.length > 1} checked={context.answerIndexes[10]?.includes(4)} value={"5"} onClick={(e) => set(4, e)} name='t11' type='checkbox' label="Sušenky" required />
      </Form>
    </>
  );
}
...
/* //nefunkční
<Form.Check readOnly {...((context.answerIndexes[10] != undefined && context.answerIndexes[10].includes(1) && { checked: true }) || { checked: false })} onClick={set(1)} name='t11' type='checkbox' label="Sekera" required />
<Form.Check readOnly {...((context.answerIndexes[10] != undefined && context.answerIndexes[10].includes(2) && { checked: true }) || { checked: false })} onClick={set(2)} name='t11' type='checkbox' label="Vysílačky" required />
<Form.Check readOnly {...((context.answerIndexes[10] != undefined && context.answerIndexes[10].includes(3) && { checked: true }) || { checked: false })} onClick={set(3)} name='t11' type='checkbox' label="Provaz" required />
<Form.Check readOnly {...((context.answerIndexes[10] != undefined && context.answerIndexes[10].includes(4) && { checked: true }) || { checked: false })} onClick={set(4)} name='t11' type='checkbox' label="Sušenky" required />
*/